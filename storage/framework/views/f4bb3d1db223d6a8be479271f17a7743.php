...
<?php $__env->startSection('content'); ?>
			...
			<div align="right">
				<a href="<?php echo e(route('add-product')); ?>"><button type="button" class="btn btn-primary mb-3">
				  Add Product
				</button></a>
			</div>
...
<script>
    ...
    var form_data = $('#category_form').validate({
        rules:{
            category:{
                required:true,
                remote:{
                    url:"<?php echo e(route('category-exists')); ?>",
                    type:"post",
                }
            },
        },messages:{
    ...
    $('#save').click(function(e){
        ...
        if($('#category_form').valid()){
            $.ajax({
            	url:"<?php echo e(route('save-category')); ?>",
                type:"post",
                data: {category : ctg},
                beforeSend: function(){
                    $('#save').text('Saving ...');
   ...
   function dltCtg(eid){
    ...
    if(cnfm){
  	    $.ajax({
            	url:"<?php echo e(route('dlt-category')); ?>",
                type:"post",
                data: {eid : eid},
                success:function(data){
                	if(data.success){
    ...
    function deleteProduct(pid){
        ...
        if(dlt){
            $.ajax({
                url:"<?php echo e(route('dlt-product')); ?>",
                type:"post",
                data:{pid:pid},
                success:function(data){
                    if(data.success){
...
<?php echo $__env->make('layouts.common', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Laravel_Mongodb\resources\views/admin/products/products.blade.php ENDPATH**/ ?>